...
/*   Updated: 2019/03/06 16:05:57 by cocummin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int mishaniabrot(void   *mlx_ptr)
{
   ...
   // static void    *mlx_ptr;
    ...
    static const char fileName[] = "mishania_kernel.cl";
    static size_t source_size;
    static char *source_str;
    static cl_mem memobj = NULL;

    static cl_mem mishania_memobj = NULL;

//-------------------------------------------------------

    t_mishania_struct mishania_struct;
    
    mishania_struct.mishania_zoom = zoom;
    mishania_struct.mishania_delta_y = delta_y;
    mishania_struct.mishania_delta_x = delta_x;
    mishania_struct.mishania_cRe = cRe;
    mishania_struct.mishania_cIm = cIm;
    mishania_struct.mishania_color = color;
    ...
    if (!(win_ptr))
    {
        ...
        kernel = clCreateKernel(program, "mishania", &ret);


        memobj = clCreateBuffer(context, CL_MEM_READ_WRITE, Width * Width * 4, NULL, &ret);
        mishania_memobj = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(t_mishania_struct), NULL, &ret);
    ...
    ret = clEnqueueWriteBuffer(command_queue, mishania_memobj, CL_TRUE, 0, sizeof(t_mishania_struct), &mishania_struct, 0, NULL, NULL);
    ret = clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *)&mishania_memobj);
